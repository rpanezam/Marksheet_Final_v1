...
function getCorsHeaders(req: Request): Record<string, string> {
  ...
  const allowedOrigin = ALLOWED_ORIGINS.includes(origin) ? origin : ALLOWED_ORIGINS[0]; // fallback to production domain
  return {
  ...
  try {
    ...
    if (action === "create") {
      ...
      if (classes?.length)
        await supabaseAdmin
          ...
          .insert(classes.map((c) => ({ user_id: uid, class_name: c })));
    ...
    if (action === "update_classes") {
      ...
      if (classes?.length)
        await supabaseAdmin
          .from("teacher_classes")
          .insert(classes.map((c) => ({ user_id, class_name: c })));
