...
import { Trash2, Plus, X, Loader2, Pencil, Eraser, ChevronDown, Eye, EyeOff, LogOut, Shield, Upload } from "lucide-react";
  ...
  async function handleSave(yr: string = year, tVal: string = term) {
    ...
    try {
      const { error } = await supabase
        .from("app_settings")
        .upsert(
          { key: APP_SETTINGS_KEYS.GLOBAL_YEAR_TERM, value: { year: yr, term: tVal }, updated_at: new Date().toISOString() },
          { onConflict: "key" },
        );
  ...
  return (
    <div className="space-y-2.5">
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-center text-[10px] font-medium text-muted-foreground">Year</label>
          <select
            value={year}
            onChange={(e) => { setYear(e.target.value); void handleSave(e.target.value, term); }}
        ...
        <div>
          <label className="block text-center text-[10px] font-medium text-muted-foreground">Term</label>
          <select
            value={term}
            onChange={(e) => { setTerm(e.target.value); void handleSave(year, e.target.value); }}
            className="mt-1 w-full rounded-lg border border-input bg-background px-2 py-2 text-xs outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
          >
...
function SchoolInfoPanel() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (v.font === "times" || v.font === "helvetica" || v.font === "courier" || v.font === "blackletter") setFont(v.font);
  ...
  async function handleSave() {
    ...
    try {
      const { error } = await supabase
        .from("app_settings")
        .upsert({ key: "school", value: { name: name.trim(), address: address.trim(), font, transcriptFont, transcriptFontSize, logoDataUrl: logoDataUrl || undefined }, updated_at: new Date().toISOString() }, { onConflict: "key" });
      if (error) throw new Error(error.message);
  ...
  return (
    <div className="space-y-2.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-[10px] font-medium text-muted-foreground">Academic Transcript font</label>
          <select
        ...
        <div>
          ...
          >
            {TRANSCRIPT_SIZE_OPTIONS.map((s) => (
              <option key={s} value={s}>{s} pt{s === 9.9 ? " (default)" : ""}</option>
            ))}
          </select>
        </div>
      </div>
...
function StudentDataRestorePanel() {
  ...
  const ALLOWED = new Set([
    "id","student_name","father_name","mother_name","student_id","class_name",
    "roll_no","exam","year_session","subject","full_marks","highest_score",
    "obtained_marks","letter_grade","gp","gpa","section_position","working_days",
    "total_present","moral_behavior","co_curricular","comments",
  ...
  const HEADER_ALIAS: Record<string, string> = {
    "student name": "student_name", "studentname": "student_name", "name": "student_name",
    "father's name": "father_name", "fathers name": "father_name", "father name": "father_name", "fathername": "father_name",
    "mother's name": "mother_name", "mothers name": "mother_name", "mother name": "mother_name", "mothername": "mother_name",
    "student id": "student_id", "studentid": "student_id", "id no": "student_id",
    "class": "class_name", "class name": "class_name", "classname": "class_name",
    "roll": "roll_no", "roll no": "roll_no", "rollno": "roll_no",
    "exam": "exam", "term": "exam",
    "year": "year_session", "session": "year_session", "year session": "year_session", "year/session": "year_session",
    "subject": "subject",
    "full marks": "full_marks", "fullmarks": "full_marks", "full": "full_marks",
    "highest": "highest_score", "highest score": "highest_score",
    "obtained": "obtained_marks", "obtained marks": "obtained_marks", "obt": "obtained_marks", "marks": "obtained_marks",
    "grade": "letter_grade", "letter grade": "letter_grade",
    "gp": "gp", "gpa": "gpa",
    "section position": "section_position", "position": "section_position",
    "working days": "working_days",
    "total present": "total_present", "present": "total_present",
    "moral": "moral_behavior", "moral behavior": "moral_behavior",
    "co curricular": "co_curricular", "co-curricular": "co_curricular", "cc": "co_curricular",
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (!raw.length) { setErr("File-এ কোনো row পাওয়া যায়নি"); return; }
  ...
  return (
    <div className="space-y-2">
      <p className="text-[11px] text-muted-foreground leading-relaxed">
        Class Data থেকে ডাউনলোড করা Excel ফাইল আপলোড করলে সব student data
        ফেরত আসবে।
      </p>
...
function SignaturesPanel() {
  ...
  async function persist(next: {
    ...
    try {
      const merged = {
        ...
        principalSigDataUrl:
          (next.principalSigDataUrl ?? principalSigDataUrl) || undefined,
        teacherSigDataUrl:
          (next.teacherSigDataUrl ?? teacherSigDataUrl) || undefined,
      };
      const { error } = await supabase
        .from("app_settings")
        .upsert({ key: "school", value: merged, updated_at: new Date().toISOString() }, { onConflict: "key" });
      if (error) throw new Error(error.message);
      setOther(merged);
      ...
      try { alert("Signature/Logo সেভ হয়নি:\n" + m); } catch { /* ignore */ }
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={async () => { if (await requireDeletePassword(`Enter password to remove ${label.toLowerCase()}:`)) onClear(); }}
  ...
  return (
    <div className="space-y-3">
      <Slot label="School logo" value={logoDataUrl} kind="logo" accept="image/*" onClear={() => { setLogoDataUrl(""); void persist({ logoDataUrl: "" }); }} />
      <div className="grid grid-cols-2 gap-2">
        <Slot label="Principal signature" value={principalSigDataUrl} kind="principal" accept="image/*" onClear={() => { setPrincipalSigDataUrl(""); void persist({ principalSigDataUrl: "" }); }} />
        <Slot label="Teacher signature" value={teacherSigDataUrl} kind="teacher" accept="image/*" onClear={() => { setTeacherSigDataUrl(""); void persist({ teacherSigDataUrl: "" }); }} />
      </div>
      <div className="flex justify-center pt-1">
        <button
          onClick={() => void handleSave()}
...
function Collapsible({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <section className="rounded-xl border border-border bg-card shadow-[var(--shadow-card)] overflow-hidden">
      ...
      >
        <span className="font-display text-[12px] font-bold uppercase tracking-[0.14em] text-foreground/80">{title}</span>
...
export function AdminPanel() {
  ...
  const [activeSessions, setActiveSessions] = useState<Record<string, { updated_at: string; allow_multi: boolean }>>({});
  const [loading, setLoading] = useState(true);
  ...
  async function handleCleanAll() {
    setCleanMsg("");
    setError(null);
    if (!(await requireDeletePassword("Enter password to clear all marksheet data (backup history is kept):"))) return;
    if (!(await confirmDialog({
      title: "Clear all marksheet records?",
      message: "All marksheet records will be permanently deleted. Backup history will remain intact.",
  ...
  async function refresh() {
    ...
    try {
      ...
      try {
        ...
        fetchedTeachers.forEach((t) => { if (t.password && !map[t.id]) map[t.id] = t.password; });
        setPasswords(map);
      } catch {
        // Fallback: use passwords from list response only
        const map: Record<string, string> = {};
        fetchedTeachers.forEach((t) => { if (t.password) map[t.id] = t.password; });
        setPasswords(map);
      }
      ...
      (sessRows ?? []).forEach((r: { user_id: string; updated_at: string; allow_multi: boolean }) => {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setUsername(""); setPassword(""); setNewClasses([]);
      await refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally {
  ...
  async function handleClassToggle(t: Teacher, cls: string) {
    const next = t.classes.includes(cls)
      ? t.classes.filter((c) => c !== cls)
      : [...t.classes, cls];
  ...
  async function handleDeletePassword(t: Teacher) {
    const teacherLabel = (t.email ?? "").replace(/@teachers\.local$/, "");
    if (!(await requireDeletePassword(`Enter password to delete ${teacherLabel}'s saved password:`))) return;
    if (!(await confirmDialog({
    ...
    try {
      ...
      setPasswords((p) => { const n = { ...p }; delete n[t.id]; return n; });
      setShown((s) => { const n = { ...s }; delete n[t.id]; return n; });
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    }
  }
  ...
  async function handleReleaseSession(t: Teacher) {
    ...
    try {
      ...
      if (e) throw new Error(e.message);
      setActiveSessions((p) => { const n = { ...p }; delete n[t.id]; return n; });
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    }
  }
      ...
      {adminTab === "teacher" && (
        <div className="space-y-3">
          <Collapsible title="Teacher Access Control">
            ...
            <div>
              <p className="text-[10px] font-medium text-muted-foreground mb-1">Assigned classes</p>
                ...
                {CLASS_OPTIONS.map((c) => {
                  ...
                  return (
                    <button
                      type="button"
                      key={c}
                      onClick={() => setNewClasses((p) => on ? p.filter((x) => x !== c) : [...p, c])}
          ...
          <Collapsible title={`Teacher Credentials (${teachers.length})`}>
            ...
            {teachers.length === 0 && !loading && (
              <p className="text-xs text-muted-foreground">Kono teacher nai. Upor theke add korun.</p>
            )}
            <ul className="space-y-2">
              {teachers.map((t) => (
                <li key={t.id} className="rounded-lg border border-border bg-background p-2">
                  <div className="flex items-center gap-2">
                    <p className="text-xs font-medium text-foreground truncate flex-shrink-0 max-w-[40%]">{(t.email ?? t.id).replace(/@teachers\.local$/, "")}</p>
                    <div className="flex flex-wrap gap-1 flex-1 min-w-0">
                      {t.classes.length === 0 ? (
                        <span className="text-[10px] text-muted-foreground italic">No class</span>
                      ) : (
                        t.classes.map((c) => (
                          <span key={c} className="rounded-full bg-primary/10 text-primary border border-primary/30 px-2 py-0.5 text-[10px] font-medium">
                            {c}
                          </span>
                        ))
                      )}
                    ...
                    >
                      <Shield className={`h-3.5 w-3.5 ${adminIds.has(t.id) ? "fill-primary/20" : ""}`} />
                    </button>
                    <button
                  ...
                  <div className="mt-2 pt-2 border-t border-border space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] uppercase tracking-wide text-muted-foreground">Password</span>
                      <code className="flex-1 truncate rounded bg-secondary/60 px-2 py-0.5 text-[11px] font-mono text-foreground">
                        {passwords[t.id] ? (shown[t.id] ? passwords[t.id] : "•".repeat(Math.min(10, passwords[t.id].length))) : <span className="italic text-muted-foreground">not stored</span>}
                      </code>
                      {passwords[t.id] ? (
                        <>
                          <button
                          ...
                          >
                            {shown[t.id] ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                          </button>
                          <button
                            type="button"
                        ...
                        <button
                          type="button"
                          onClick={() => { setSettingPasswordId(t.id); setNewPassword(""); }}
                    ...
                    {settingPasswordId === t.id && (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onKeyDown={(e) => { if (e.key === "Enter") void handleSetPassword(t); if (e.key === "Escape") setSettingPasswordId(null); }}
                          autoFocus
                        />
                        <button
                          type="button"
                  ...
                  {(() => {
                    ...
                    const isLive = sess && (Date.now() - new Date(sess.updated_at).getTime() < 90_000);
                    return (
                      <div className="mt-2 flex items-center gap-2 pt-2 border-t border-border">
                        ...
                        <span className={`flex-1 text-[11px] font-medium ${isLive ? "text-primary" : "text-muted-foreground"}`}>
                        ...
                        {sess && (
                          ...
                          >
                            <LogOut className="h-3 w-3 inline mr-1" />Release
                          </button>
                        )}
                      </div>
                    );
        ...
        <div className="space-y-2">
          <button
            ...
            className="w-full inline-flex items-center justify-center gap-1.5 rounded-lg border border-destructive/40 bg-background px-3 py-2 text-[11px] font-semibold text-destructive hover:bg-destructive/10 transition-colors disabled:opacity-40"
          >
            {forcingOut ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <LogOut className="h-3.5 w-3.5" />}
